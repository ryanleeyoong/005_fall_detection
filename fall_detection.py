...
while True:
    ...
    for idx in range(output.shape[0]):
        plot_skeleton_kpts(nimg, output[idx, 7:], 3)

        # Check if all keypoints are below the horizontal line
        keypoints = output[idx, 7:]  # Extract keypoints (flattened array)
        keypoints_y = keypoints[1::3]  # Extract y-coordinates (every third value starting from index 1)
        if np.all(keypoints_y > 480):
            fall_detected = True

    # Draw the horizontal line
    cv2.line(nimg, (0, 480), (960, 480), (255, 0, 0), 2)

    # Display fall detection message
    if fall_detected:
        cv2.putText(nimg, "Fall Detected!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 3)

    cv2.putText(nimg, "Press 'q' to exit program", (300, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
